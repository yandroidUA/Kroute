...
import io.github.yanadroidua.kroute.lifecycle.koin.TestDiScopeLifecycleStackListener.TestRoute.WithScope
import io.github.yanadroidua.kroute.lifecycle.koin.TestDiScopeLifecycleStackListener.TestRoute.WithoutScope
...
class TestDiScopeLifecycleStackListener : KoinTest {
    ...
    fun should_not_do_anything_if_no_scope_associated_in_onPopped() {
        ...
        diScopeLifecycleStackListener.onPopped(
            popped = listOf(Transition(id = "TEST", destination = WithoutScope)),
            stack = emptyList(),
        )
    ...
    fun should_not_do_anything_if_no_scope_in_onPopped() {
        ...
        diScopeLifecycleStackListener.onPopped(
            popped = listOf(Transition(id = "TEST", destination = WithScope)),
            stack = emptyList(),
        )
